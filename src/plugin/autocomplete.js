...
class TeleportAutocomplete {
  ...
  getCities(cb) {
    ...
    req.open(
      "GET",
      `${this.apiRoot}/cities/?search=${query}&embed=${embed}&limit=${this.maxItems}`
    );
    req.setRequestHeader(
      "Accept",
      `application/vnd.teleport.v${this.apiVersion}+json`
  ...
  parseCity(res) {
    ...
    if (city.admin1_division) {
      const { name: admin1Division, geonames_admin1_code: admin1DivisionCode } =
