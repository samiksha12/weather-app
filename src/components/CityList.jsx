...
function CityList() {
  ...
  const handleDelete = (data, geonameId) => {
    dispatch(deleteCityApiAction(data, geonameId, user , instance));
  };
  ...
  function getTemperature(temp, is_celcius) {
    if (is_celcius === "farenhite") {
      const temperature = changeTemperature(Math.round(temp), "farenhite");
      const icon = "\u00b0F";
      ...
      <ul className="list-group list-group-flush" id="list-group-city">
        {cityData.data &&
          ...
          sortedData.map((list, index) => {
            const { temperature, icon } = getTemperature(
              Math.round(list.current_weather?.temperature),
              isCelcius.is_celcius
            );
            const { year, month, day, hours, minutes, suffix } = getDate(
              list.current_weather?.time
            );
            const weatherIcon =
              list.current_weather !== undefined
                ? weatherCode[list.current_weather?.weathercode][
                    "weather-icon"
                  ][list.current_weather?.is_day]
                : "pe-is-w-sun-1";
            return (
              <a
                href="#"
              ...
              >
                <div className="ms-2 me-auto">
                  ...
                  <div className="last-update">
                    Last updated : {month} {day} {year} , {hours}:{minutes}
                    {suffix}
                  </div>
                </div>
                <span className="px-2">
                  <i className={`pe-2x ${weatherIcon}`}></i>
                </span>
                <span className="temperature1">
                  {isNaN(temperature) ? "N/A" : temperature}
                </span>
                <span className="degree-icon">{icon}</span>
                {(list.active === true) ? (<button
